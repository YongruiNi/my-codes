...
## of 41000 obs. 3 yes for default. Giving errors in model prediction. 
df <- subset(df, default != "yes")
df$default <- droplevels(df)$default
...
getp=function(mod,train){# return the best cut-probility
  ...
  for(i in 1:71){
    ...
    error[i]=sum(pre!=train$y)
  }
  return(mean(p[which(error==min(error))]))
}

## assign p and predict
p=getp(xy.glm,df.train)
...
##
## Use the kNN classifier with k = 5
...
contDur.knn = knn(
  train = df.train[c("contact","duration", "marital")], # training data for features used in classification
  test = df.test[c("contact","duration", "marital")], # test data data for features used in classification
...
cbind(df.test,contDur.knn)

contDur.knn.conf = table(true = df.test$y, predicted = contDur.knn)

################################################
## Plotting features
## compare various significant features:
library(ggplot2)


## 2 Bar plots
job <- ggplot(df, aes(job)) + geom_bar(aes(fill= y)) +
  labs(title="Subscription by Employment",x="Employment Type", y = "Count", fill= "Subscribed")


education <- ggplot(df, aes(education)) + geom_bar(aes(fill=y)) +
  labs(title="Subscription by Education",x="Education", y = "Count", fill= "Subscribed")



## 3 violin plots (1 no fill)
previousAge <- ggplot(df, aes(x=previous, y=age, fill = y)) + 
  geom_violin() +
  labs(title="Number of prior contacts and Age",x="Conacts", y = "Age", fill= "Subscribed")



age <- ggplot(df, aes(x=y, y=age)) + 
  geom_violin() +
  labs(title="Number of prior contacts and Age",x="Subscribed", y = "Age")
